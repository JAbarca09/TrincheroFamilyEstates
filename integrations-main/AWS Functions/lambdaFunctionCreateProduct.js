exports.handler = async (event) => {
    ...
    REMOVED
    group_name: c7payload.payload.metaData['yotpo-product-grouping'],*/
    ...
    await fetch('https://api.yotpo.com/core/v3/stores/storeID/access_tokens', generateUToken)
        ...
        .then(UTokenData => {
            ...
            console.log(options);
            //creates a yotpo Product
            fetch('https://api.yotpo.com/core/v3/stores/storeID/products', options)
                .then(response => response.json())
    ...
    let commerce7UpdateProductHeader = {
        ...
        method: 'PUT',
        headers: {
            'tenant': 'one-stop-wine-shop', 'Content-Type': 'application/json', "Authorization": "key"
        },
        body: JSON.stringify({
            metaData: {
                "yotpo-star-rating": `<div class=\"yotpo bottomLine\" data-yotpo-product-id=\"${c7payload.payload.variants[0].sku}\"></div>`,
                "yotpo-review-widget": `<div class=\"yotpo yotpo-main-widget\" data-product-id=\"${c7payload.payload.variants[0].sku}\" data-price=\"${c7payload.payload.variants[0].price / 100}\" data-currency=\"USD\" data-name=\"${c7payload.payload.title}\" data-url=\"https://onestopwineshop.com/product/${c7payload.payload.slug}\" data-image-url=\"${c7payload.payload.image}\"></div>`
            }
        })
    };
    ...
    await fetch(`https://api.commerce7.com/v1/product/${c7payload.payload.id}`, commerce7UpdateProductHeader)
        .then(response => response.json())
        .then(response => console.log(response))
        .catch(err => console.error(err));
